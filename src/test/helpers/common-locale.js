...
export function defineCommonLocaleTests(locale) {
    ...
    test('meridiem invariant', function(assert) {
        ...
        for (h = 0; h < 24; ++h) {
            for (m = 0; m < 60; m += 15) {
                ...
                assert.equal(t2.format('HH:mm'), t1.format('HH:mm'), 'meridiem at ' + t1.format('HH:mm'));
            }
        }
    });
    ...
    test('date format correctness', function(assert) {
        ...
        each(tokens, function(srchToken) {
            ...
            each(tokens, function(baseToken) {
                ...
                assert.equal(false, !!~format.indexOf(srchToken), 'contains ' + srchToken + ' in ' + baseToken);
            });
        });
    });
    ...
    test('weekday parsing correctness', function(assert) {
        ...
        function tester(format) {
            var r,
                baseMsg = 'weekday ' + m.weekday() + ' fmt ' + format + ' ' + m.toISOString();
            r = moment(m.format(format), format);
