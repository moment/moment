...
test('format week', function(assert) {
    var expected = 'អាទិត្យ អា អា_ច័ន្ទ ច ច_អង្គារ អ អ_ពុធ ព ព_ព្រហស្បតិ៍ ព្រ ព្រ_សុក្រ សុ សុ_សៅរ៍ ស ស'.split('_'),
        i;
    for (i = 0; i < expected.length; i++) {
...
test('from', function(assert) {
    ...
    assert.equal(start.from(moment([2007, 1, 28]).add({ s: 90 }), true), '២ នាទី', '90 seconds = 2 នាទី');
    assert.equal(start.from(moment([2007, 1, 28]).add({ m: 44 }), true), '៤៤ នាទី', '44 minutes = 44 នាទី');
    ...
    assert.equal(start.from(moment([2007, 1, 28]).add({ m: 90 }), true), '២ ម៉ោង', '90 minutes = 2 ម៉ោង');
    assert.equal(start.from(moment([2007, 1, 28]).add({ h: 5 }), true), '៥ ម៉ោង', '5 hours = 5 ម៉ោង');
    assert.equal(start.from(moment([2007, 1, 28]).add({ h: 21 }), true), '២១ ម៉ោង', '21 hours = 21 ម៉ោង');
...
test('weeks year starting sunday formatted', function(assert) {
    assert.equal(moment([2012, 0, 1]).format('w ww wo'), '៥២ ៥២ ទី៥២', 'Jan  1 2012 should be week 52');
