...
test('parse strict no dots', function (assert) {
    var tests = 'Jänner Jän_Februar Febr_März Mrz_April Apr_Mai Mai_Juni Jun_Juli Jul_August Aug_September Sept_Oktober Okt_November Nov_Dezember Dez'.split('_'), i;
    function equalTest(input, mmm, i) {
        assert.equal(moment(input, mmm, true).month(), i, input + ' should be month ' + (i + 1));
    }
    for (i = 0; i < 12; i++) {
