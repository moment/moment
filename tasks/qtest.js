    ...
    grunt.task.registerTask('qtest', 'run tests locally', function () {
        ...
        if (grunt.option('only') != null) {
            tests = grunt.file.expand.apply(null, grunt.option('only').split(',').map(function (file) {
                if (file === 'moment') {
                    return 'build/test/moment/*.js';
                } else if (file === 'locale') {
                    return 'build/test/locale/*.js';
                } else {
                    return 'build/test/' + file + '.js';
                }
            }));
        } else {
            tests = grunt.file.expand('build/test/moment/*.js',
                'build/test/locale/*.js');
        }

        testrunner.run({
            code: 'build/moment.js',
            tests: tests
        }, function (err, report) {
            if (err) {
