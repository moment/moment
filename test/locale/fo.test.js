...
describe('locale:fo', () => {
    ...
    test('suffix', () => {
        expect(moment(30000).from(0), 'prefix').toBe('um fá sekund');
        expect(moment(0).from(30000), 'suffix').toBe('fá sekund síðani');
    });
    ...
    test('calendar last week', () => {
        ...
        for (i = 2; i < 7; i++) {
            ...
            expect(
                m.calendar(),
                'today - ' + i + ' days beginning of day'
            ).toBe(m.format('[síðstu] dddd [kl] LT'));
            m.hours(23).minutes(59).seconds(59).milliseconds(999);
            expect(m.calendar(), 'today - ' + i + ' days end of day').toBe(
                m.format('[síðstu] dddd [kl] LT')
            );
