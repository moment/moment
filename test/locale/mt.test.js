...
describe('locale:mt', () => {
    ...
    test('suffix', () => {
        expect(moment(30000).from(0), 'prefix').toBe('f’ ftit sekondi');
        expect(moment(0).from(30000), 'suffix').toBe('ftit sekondi ilu');
    });
    ...
    test('calendar day', () => {
        ...
        ).toBe('Għada fil-12:00');
        expect(
            moment(a).subtract({ h: 1 }).calendar(),
            'Now minus 1 hour'
        ).toBe('Illum fil-11:00');
        expect(
            moment(a).subtract({ d: 1 }).calendar(),
            'yesterday at the same time'
        ).toBe('Il-bieraħ fil-12:00');
    });
    ...
    test('calendar last week', () => {
        ...
        for (i = 2; i < 7; i++) {
            ...
            expect(
                m.calendar(),
                'Today - ' + i + ' days beginning of day'
            ).toBe(m.format('dddd [li għadda] [fil-]LT'));
            m.hours(23).minutes(59).seconds(59).milliseconds(999);
            expect(m.calendar(), 'Today - ' + i + ' days end of day').toBe(
                m.format('dddd [li għadda] [fil-]LT')
            );
