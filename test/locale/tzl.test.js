...
import '../../src/locale/tzl';

describe('locale:tzl', () => {
    setupLocaleTests('tzl');
    ...
    test('suffix', () => {
        expect(moment(30000).from(0), 'prefix').toBe('osprei viensas secunds');
        expect(moment(0).from(30000), 'suffix').toBe("ja'iensas secunds");
    });
    ...
    test('calendar last week', () => {
        ...
        for (i = 2; i < 7; i++) {
            ...
            expect(
                m.calendar(),
                'Today - ' + i + ' days beginning of day'
            ).toBe(m.format('[sür el] dddd [lasteu à] LT'));
            m.hours(23).minutes(59).seconds(59).milliseconds(999);
            expect(m.calendar(), 'Today - ' + i + ' days end of day').toBe(
                m.format('[sür el] dddd [lasteu à] LT')
            );
