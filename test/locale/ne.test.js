...
describe('locale:ne', () => {
    ...
    test('suffix', () => {
        expect(moment(30000).from(0), 'prefix').toBe('केही क्षणमा');
        expect(moment(0).from(30000), 'suffix').toBe('केही क्षण अगाडि');
    });
    ...
    test('now from now', () => {
        ...
        ).toBe('केही क्षण अगाडि');
    });

    test('fromNow', () => {
        expect(moment().add({ s: 30 }).fromNow(), 'केही क्षणमा').toBe(
            'केही क्षणमा'
        );
        expect(moment().add({ d: 5 }).fromNow(), '५ दिनमा').toBe('५ दिनमा');
    });
    ...
    test('calendar day', () => {
        ...
        ).toBe('भोलि दिउँसोको १२:०० बजे');
        expect(
            moment(a).subtract({ h: 1 }).calendar(),
            'Now minus 1 hour'
        ).toBe('आज बिहानको ११:०० बजे');
        expect(
            moment(a).subtract({ d: 1 }).calendar(),
            'yesterday at the same time'
        ).toBe('हिजो दिउँसोको १२:०० बजे');
    });
    ...
    test('meridiem', () => {
        ...
        expect(moment([2011, 2, 23, 21, 20]).format('a'), 'night').toBe('राति');
