...
describe('locale:ga', () => {
    ...
    test('from', () => {
        ...
        expect(
            ...
            '90 minutes = 2 hours'
        ).toBe('2 uair an chloig');
        expect(
            start.from(moment([2007, 1, 28]).add({ h: 5 }), true),
            '5 hours = 5 hours'
        ).toBe('5 uair an chloig');
        expect(
            start.from(moment([2007, 1, 28]).add({ h: 21 }), true),
            '21 hours = 21 hours'
        ).toBe('21 uair an chloig');
        expect(
    ...
    test('suffix', () => {
        expect(moment(30000).from(0), 'prefix').toBe('i cúpla soicind');
        expect(moment(0).from(30000), 'suffix').toBe('cúpla soicind ó shin');
    });
    ...
    test('fromNow', () => {
        ...
        expect(moment().add({ d: 5 }).fromNow(), 'in 5 days').toBe('i 5 lá');
    });
    ...
    test('calendar last week', () => {
        ...
        for (i = 2; i < 7; i++) {
            ...
            expect(
                m.calendar(),
                'Today - ' + i + ' days beginning of day'
            ).toBe(m.format('dddd [seo caite] [ag] LT'));
            m.hours(23).minutes(59).seconds(59).milliseconds(999);
            expect(m.calendar(), 'Today - ' + i + ' days end of day').toBe(
                m.format('dddd [seo caite] [ag] LT')
            );
