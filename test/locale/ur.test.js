...
describe('locale:ur', () => {
    ...
    test('suffix', () => {
        expect(moment(30000).from(0), 'prefix').toBe('چند سیکنڈ بعد');
        expect(moment(0).from(30000), 'suffix').toBe('چند سیکنڈ قبل');
    });
    ...
    test('fromNow', () => {
        ...
        expect(moment().add({ d: 5 }).fromNow(), 'in 5 days').toBe('5 دن بعد');
    });
    ...
    test('calendar last week', () => {
        ...
        for (i = 2; i < 7; i++) {
            ...
            expect(
                m.calendar(),
                'Today - ' + i + ' days beginning of day'
            ).toBe(m.format('[گذشتہ] dddd [بوقت] LT'));
            m.hours(23).minutes(59).seconds(59).milliseconds(999);
            expect(m.calendar(), 'Today - ' + i + ' days end of day').toBe(
                m.format('[گذشتہ] dddd [بوقت] LT')
            );
