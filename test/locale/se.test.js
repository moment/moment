...
describe('locale:se', () => {
    ...
    test('suffix', () => {
        expect(moment(30000).from(0), 'prefix').toBe('moadde sekunddat geažes');
        expect(moment(0).from(30000), 'suffix').toBe('maŋit moadde sekunddat');
    });
    ...
    test('calendar last week', () => {
        ...
        for (i = 2; i < 7; i++) {
            ...
            expect(
                m.calendar(),
                'Today - ' + i + ' days beginning of day'
            ).toBe(m.format('[ovddit] dddd [ti] LT'));
            m.hours(23).minutes(59).seconds(59).milliseconds(999);
            expect(m.calendar(), 'Today - ' + i + ' days end of day').toBe(
                m.format('[ovddit] dddd [ti] LT')
            );
