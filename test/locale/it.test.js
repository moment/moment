...
describe('locale:it', () => {
    ...
    test('suffix', () => {
        expect(moment(30000).from(0), 'prefix').toBe('tra alcuni secondi');
        expect(moment(0).from(30000), 'suffix').toBe('alcuni secondi fa');
    });

    test('fromNow', () => {
        expect(moment().add({ s: 30 }).fromNow(), 'tra seconds').toBe(
            'tra alcuni secondi'
        );
    ...
    test('calendar day', () => {
        ...
        ).toBe('Domani alle 12:00');
        expect(
            moment(a).add({ d: 1, h: -1 }).calendar(),
            'tomorrow minus 1 hour'
        ).toBe('Domani alle 11:00');
        expect(
            moment(a).subtract({ h: 1 }).calendar(),
            'Now minus 1 hour'
        ).toBe('Oggi alle 11:00');
        expect(
            moment(a).subtract({ d: 1 }).calendar(),
            'yesterday at the same time'
        ).toBe('Ieri alle 12:00');
    });
