...
import '../../src/locale/x-pseudo';

describe('locale:x-pseudo', () => {
    setupLocaleTests('x-pseudo');
    ...
    test('suffix', () => {
        expect(moment(30000).from(0), 'prefix').toBe('í~ñ á ~féw ~sécó~ñds');
        expect(moment(0).from(30000), 'suffix').toBe('á ~féw ~sécó~ñds á~gó');
    });
    ...
    test('calendar day', () => {
        ...
        ).toBe('T~ómó~rró~w át 12:00');
        expect(
            moment(a).subtract({ h: 1 }).calendar(),
            'Now minus 1 hour'
        ).toBe('T~ódá~ý át 11:00');
        expect(
            moment(a).subtract({ d: 1 }).calendar(),
            'yesterday at the same time'
        ).toBe('Ý~ést~érdá~ý át 12:00');
    });
    ...
    test('calendar last week', () => {
        ...
        for (i = 2; i < 7; i++) {
            ...
            expect(
                m.calendar(),
                'Today - ' + i + ' days beginning of day'
            ).toBe(m.format('[L~ást] dddd [át] LT'));
            m.hours(23).minutes(59).seconds(59).milliseconds(999);
            expect(m.calendar(), 'Today - ' + i + ' days end of day').toBe(
                m.format('[L~ást] dddd [át] LT')
            );
