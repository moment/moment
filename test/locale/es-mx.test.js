...
import '../../src/locale/es-mx';

describe('locale:es-mx', () => {
    setupLocaleTests('es-mx');
    ...
    test('suffix', () => {
        expect(moment(30000).from(0), 'prefix').toBe('en unos segundos');
        expect(moment(0).from(30000), 'suffix').toBe('hace unos segundos');
    });
    ...
    test('fromNow', () => {
        expect(moment().add({ s: 30 }).fromNow(), 'en unos segundos').toBe(
            'en unos segundos'
        );
        expect(moment().add({ d: 5 }).fromNow(), 'en 5 días').toBe('en 5 días');
    });
    ...
    test('calendar day', () => {
        ...
        ).toBe('mañana a las 12:00');
        expect(
            moment(a).add({ d: 1, h: -1 }).calendar(),
            'tomorrow minus 1 hour'
        ).toBe('mañana a las 11:00');
        expect(
            moment(a).subtract({ h: 1 }).calendar(),
            'Now minus 1 hour'
        ).toBe('hoy a las 11:00');
        expect(
            moment(a).subtract({ d: 1 }).calendar(),
            'yesterday at the same time'
        ).toBe('ayer a las 12:00');
    });
