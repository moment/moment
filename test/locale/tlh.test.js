...
import '../../src/locale/tlh';

describe('locale:tlh', () => {
    setupLocaleTests('tlh');
    ...
    test('from', () => {
        ...
        expect(
            ...
            '112 years = 112 years'
        ).toBe('wa’vatlh wa’maH cha’ DIS');
        expect(
            start.from(moment([2007, 1, 28]).add({ y: 122 }), true),
            '122 years = 122 years'
        ).toBe('wa’vatlh cha’maH cha’ DIS');
        expect(
            start.from(moment([2007, 1, 28]).add({ y: 213 }), true),
            '213 years = 213 years'
        ).toBe('cha’vatlh wa’maH wej DIS');
        expect(
            start.from(moment([2007, 1, 28]).add({ y: 223 }), true),
            '223 years = 223 years'
        ).toBe('cha’vatlh cha’maH wej DIS');
    });

    test('suffix', () => {
        expect(moment(30000).from(0), 'suffix').toBe('puS lup pIq');
        expect(moment(0).from(30000), 'suffix').toBe('puS lup ret');
    });
    ...
    test('fromNow', () => {
        ...
        expect(moment().add({ d: 5 }).fromNow(), 'in 5 days').toBe('vagh leS');
        expect(moment().add({ M: 2 }).fromNow(), 'in 2 months').toBe(
            'cha’ waQ'
        );
        expect(moment().add({ y: 1 }).fromNow(), 'in a year').toBe('wa’ nem');
        expect(moment().add({ s: -30 }).fromNow(), 'a few seconds ago').toBe(
            'puS lup ret'
        );
        expect(moment().add({ h: -1 }).fromNow(), 'an hour ago').toBe(
            'wa’ rep ret'
        );
        expect(moment().add({ d: -5 }).fromNow(), '5 days ago').toBe(
            'vagh Hu’'
        );
        expect(moment().add({ M: -2 }).fromNow(), '2 months ago').toBe(
            'cha’ wen'
        );
        expect(moment().add({ y: -1 }).fromNow(), 'a year ago').toBe('wa’ ben');
    });
