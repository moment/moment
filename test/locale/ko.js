...
exports['locale:ko'] = {
    ...
    'from' : function (test) {
        ...
        test.equal(start.from(moment([2007, 1, 28]).add({s: 44}), true),  '잠시', '44초 = 잠시');
        test.equal(start.from(moment([2007, 1, 28]).add({s: 45}), true),  '1분',      '45초 = 1분');
        test.equal(start.from(moment([2007, 1, 28]).add({s: 89}), true),  '1분',      '89초 = 1분');
        ...
        test.equal(start.from(moment([2007, 1, 28]).add({m: 45}), true),  '1시간',       '45분 = 1시간');
        test.equal(start.from(moment([2007, 1, 28]).add({m: 89}), true),  '1시간',       '89분 = 1시간');
        ...
        test.equal(start.from(moment([2007, 1, 28]).add({h: 22}), true),  '1일',         '22시간 = 1일');
        test.equal(start.from(moment([2007, 1, 28]).add({h: 35}), true),  '1일',         '35시간 = 1일');
        test.equal(start.from(moment([2007, 1, 28]).add({h: 36}), true),  '2일',        '36시간 = 2일');
        test.equal(start.from(moment([2007, 1, 28]).add({d: 1}), true),   '1일',         '1일 = 1일');
        ...
        test.equal(start.from(moment([2007, 1, 28]).add({d: 26}), true),  '한 달',       '26일 = 한 달');
        test.equal(start.from(moment([2007, 1, 28]).add({d: 30}), true),  '한 달',       '30일 = 한 달');
        test.equal(start.from(moment([2007, 1, 28]).add({d: 43}), true),  '한 달',       '45일 = 한 달');
        test.equal(start.from(moment([2007, 1, 28]).add({d: 46}), true),  '2개월',      '46일 = 2개월');
        test.equal(start.from(moment([2007, 1, 28]).add({d: 74}), true),  '2개월',      '75일 = 2개월');
        test.equal(start.from(moment([2007, 1, 28]).add({d: 76}), true),  '3개월',      '76일 = 3개월');
        test.equal(start.from(moment([2007, 1, 28]).add({M: 1}), true),   '한 달',       '1개월 = 한 달');
        test.equal(start.from(moment([2007, 1, 28]).add({M: 5}), true),   '5개월',      '5달 = 5개월');
        test.equal(start.from(moment([2007, 1, 28]).add({d: 345}), true), '1년',        '345일 = 1년');
        test.equal(start.from(moment([2007, 1, 28]).add({d: 548}), true), '2년',       '548일 = 2년');
        test.equal(start.from(moment([2007, 1, 28]).add({y: 1}), true),   '1년',        '1년 = 1년');
        test.equal(start.from(moment([2007, 1, 28]).add({y: 5}), true),   '5년',       '5년 = 5년');
        test.done();
    },

    'suffix' : function (test) {
        test.equal(moment(30000).from(0), '잠시 후',  'prefix');
        test.equal(moment(0).from(30000), '잠시 전', 'suffix');

        test.done();
    },

    'now from now' : function (test) {
        test.equal(moment().fromNow(), '잠시 전',  'now from now should display as in the past');

        test.done();
    },

    'fromNow' : function (test) {
        test.equal(moment().add({s: 30}).fromNow(), '잠시 후', 'in a few seconds');
    ...
    'calendar last week' : function (test) {
        ...
        for (i = 2; i < 7; i++) {
            ...
            test.equal(m.calendar(),       m.format('지난 주 dddd LT'),  'Today - ' + i + ' days current time');
            m.hours(0).minutes(0).seconds(0).milliseconds(0);
            test.equal(m.calendar(),       m.format('지난 주 dddd LT'),  'Today - ' + i + ' days beginning of day');
            m.hours(23).minutes(59).seconds(59).milliseconds(999);
            test.equal(m.calendar(),       m.format('지난 주 dddd LT'),  'Today - ' + i + ' days end of day');
        }
        test.done();
    },
